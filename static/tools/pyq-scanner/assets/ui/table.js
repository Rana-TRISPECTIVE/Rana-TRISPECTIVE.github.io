...
export class PYQTable {
  ...
  renderRows() {
    ...
    // Direct rendering for standard datasets (<= 100 rows) ensures zero scroll bounce/oscillation
    if (totalCount <= 100) {
    ...
    // --- VIRTUALIZATION ENGINE (for large datasets > 100 rows) ---
    const visibleCount = Math.ceil((this.containerHeight || 600) / this.estimatedRowHeight);

    // Clamp current scroll position to avoid phantom overflow bounds
    const maxPossibleScroll = Math.max(0, (totalCount * this.estimatedRowHeight) - (this.containerHeight || 600));
    if (this.scrollTop > maxPossibleScroll) {
      this.scrollTop = maxPossibleScroll;
      if (this.tableScrollContainer && this.tableScrollContainer.scrollTop !== maxPossibleScroll) {
        this.tableScrollContainer.scrollTop = maxPossibleScroll;
      }
    }

    let startIndex = Math.max(0, Math.floor(this.scrollTop / this.estimatedRowHeight) - this.overscan);
