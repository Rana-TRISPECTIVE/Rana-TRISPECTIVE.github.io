...
import { getDeterministicColor } from './editor.js';
...
export function openColumnDialog(existingColumn = null) {
  ...
  const saveBtn = el('button', {
    ...
    onclick: async () => {
      ...
      if (isEditing) {
        // Update column definition & auto-harvest tag options with distinct colors
        let options = Array.isArray(existingColumn.options) ? [...existingColumn.options] : [];
        if (type === COLUMN_TYPES.SELECT) {
          const knownLabels = new Set(options.map(o => (o.label || '').toLowerCase()));
          rows.forEach(r => {
            const val = r.cells ? r.cells[existingColumn.id] : null;
            if (val) {
              String(val).split(/[,;]/).forEach(rawT => {
                const t = rawT.trim();
                if (t && !knownLabels.has(t.toLowerCase())) {
                  knownLabels.add(t.toLowerCase());
                  const cfg = getDeterministicColor(t);
                  options.push({ label: t, color: cfg.name });
                }
              });
            }
          });
        ...
        const nextCols = columns.map(c => c.id === existingColumn.id ? {
          ...c,
          name,
          type,
          options: type === COLUMN_TYPES.SELECT ? options : c.options
        } : c);
