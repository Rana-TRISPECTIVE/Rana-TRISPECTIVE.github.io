    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (filters && filters.length > 0) {
        const pass = filters.map(f => {
          const val = row.cells ? String(row.cells[f.columnId] || '').toLowerCase() : '';
          const rVal = String(f.value || '').toLowerCase().trim();
          switch (f.operator) {
            case 'equals': return val === rVal;
            case 'not_equals': return val !== rVal;
            case 'contains': return val.includes(rVal);
            case 'not_contains': return !val.includes(rVal);
